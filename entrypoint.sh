...
DOCKER_HOST_HOME=/var/lib/kubelet/pods/`kubectl get pod $HOSTNAME -o 'jsonpath={.metadata.uid}'`/volumes/kubernetes.io~empty-dir/workspace-volume
DOCKER_HOST_PWD=${PWD/$HOME/$DOCKER_HOST_HOME}

echo -e "Current jenkins agent's docker host path is: $DOCKER_HOST_PWD\nUse this path when docker volume mounting, for example \${PWD/\$HOME/\$DOCKER_HOST_HOME} can be used to replace \$PWD variable"
